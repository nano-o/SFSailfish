...
* Then you can run the TLC model-checker with `make run-tlc TLA_SPEC=TLCSailfish1.tla` or `make run-tlc TLA_SPEC=TLCSailfish2.tla`. `TLCSailfish1.tla` imports `Sailfish.tla` and provides concrete definitions for model-checking a configuration with 3 nodes, among which one is Byzantine, and 5 protocol rounds (and `TLCSailfish1.cfg` specifies what to check). `TLCSailfish2` is similar but for 4 nodes.

On a recent desktop machine, model-checking `TLCSailfish1` takes about 30 seconds and explores 953,442 distinct states. Model-checking `TLCSailfish2` takes hours.
